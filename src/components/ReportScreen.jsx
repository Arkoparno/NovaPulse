...
function generatePDF(results, patientInfo, metrics, ranges) {
  ...
  doc.text('NovaPulse Clinical Report', M, 11);
  ...
  doc.text('NovaPulse is an experimental platform. Results are indicative only and not a medical diagnosis.', M + 4, anchorY + 9.5);
  
  const qualityStr = `Quality Confidence: ${results.confidence}%`;
  doc.text(qualityStr, W - M - doc.getTextWidth(qualityStr) - 4, anchorY + 9.5);

  const filename = patientInfo && patientInfo.name ? `NovaPulse_${patientInfo.name.replace(/\s+/g,'_')}_${Date.now()}.pdf` : `NovaPulse_Report_${Date.now()}.pdf`;
  doc.save(filename);
}
...
function generateCopyText(results, patientInfo, metrics, ranges) {
  ...
  return [
    ...
    'NovaPulse — Contactless Health Research',
  ].join('\n');
}
...
export function ReportScreen({
  ...
  useEffect(() => {
    console.log('[NovaPulse Debug] Results received:', results);
    console.log('[NovaPulse Debug] HR:', results?.hr);
    console.log('[NovaPulse Debug] RMSSD:', results?.rmssd);
    console.log('[NovaPulse Debug] IBIs count:', results?.ibis?.length);
    console.log('[NovaPulse Debug] Signal length:', results?.signal?.length);
    console.log('[NovaPulse Debug] PI:', results?.pi);
    console.log('[NovaPulse Debug] Confidence:', results?.confidence);
    const autonomicTest = results?.ibis?.length >= 15 ? computeAutonomic(results.ibis) : null;
    console.log('[NovaPulse Debug] Autonomic result:', autonomicTest);
  }, [results]);
  ...
  try {
    ...
    console.error('[NovaPulse] CRITICAL: Metric derivation failed:', err);
    ...
    <div className="report-layout">
      ...
      <aside className="report-sidebar">
        <div className="sidebar-header">
          <div className="sidebar-logo">NovaPulse</div>
